...
NeuOrientation computeNeuOrientation(const Vector3d& acceleration,
    ...
    if (!acceleration.allFinite() || !calibrated_magnetometer.allFinite() ||
        acceleration.norm() < 1e-9 || calibrated_magnetometer.norm() < 1e-9) {
        return result;
    }

    const Vector3d up = acceleration.normalized();
    Vector3d north = calibrated_magnetometer - up * up.dot(calibrated_magnetometer);
    if (!north.allFinite() || north.norm() < 1e-9) return result;
    north.normalize();
